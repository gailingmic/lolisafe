...
panel.prepareDashboard = () => {
  ...
  document.getElementById("itemLogout").innerHTML = `Logout ( ${
    panel.username
  } )`;
        ...
        for (const item of response.data.files) {
          ...
          if (item.thumb !== undefined)
            ...
            div.innerHTML = `<a href="${
              item.file
            }" target="_blank"><h1 class="title">.${item.file
              .split(".")
        ...
        for (const item of response.data.files) {
          ...
          tr.innerHTML = `
					<tr>
						...
						<td>${item.date}</td>
						<td>
							<a class="button is-small is-danger is-outlined" title="Delete album" onclick="panel.deleteFile(${
                item.id
              })">
								<span class="icon is-small">
      ...
      for (const item of response.data.albums) {
        ...
        tr.innerHTML = `
				<tr>
					...
					<td>
						<a class="button is-small is-primary is-outlined" title="Edit name" onclick="panel.renameAlbum(${
              item.id
            })">
							<span class="icon is-small">
						...
						<a class="button is-small is-danger is-outlined" title="Delete album" onclick="panel.deleteAlbum(${
              item.id
            })">
							<span class="icon is-small">
								<i class="fa fa-trash-o"></i>
        ...
        .then(response => {
          if (response.data.success === false) {
            if (response.data.description === "No token provided")
              ...
              swal.showInputError("That name is already in use!");
          ...
          swal(
            "Success!",
            `Your album was renamed to: ${inputValue}`,
            "success"
          );
          panel.getAlbumsSidebar();
          panel.getAlbums();
...
panel.changePassword = () => {
  ...
  document
    .getElementById("sendChangePassword")
