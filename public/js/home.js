...
upload.preparePage = () => {
  ...
  if (!upload.token)
    return (document.getElementById("loginToUpload").style.display =
      "inline-flex");
...
upload.prepareUpload = () => {
  ...
  document.getElementById(
    "maxFileSize"
  ).innerHTML = `Maximum upload size per file is${upload.maxFileSize}`;
  document.getElementById("loginToUpload").style.display = "none";

  if (upload.token === undefined)
    document.getElementById("loginLinkText").innerHTML =
...
upload.prepareShareX = () => {
  if (upload.token) {
    const sharex_element = document.getElementById("ShareX");
    const sharex_file = `{
    ...
    const sharex_blob = new Blob([sharex_file], {
...
window.addEventListener("paste", event => {
  ...
  for (index in items) {
    ...
    if (item.kind === "file") {
      ...
      const file = new File(
        [blob],
        "pasted-image." + blob.type.match(/(?:[^\/]*\/)([^;]*)/)[1]
      );
      file.type = blob.type;
      console.log(file);
