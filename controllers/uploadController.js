...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const access = i => {
      const name =
        randomstring.generate(config.uploads.fileLength) +
        path.extname(file.originalname);
      fs.access(path.join(uploadDir, name), err => {
        if (err) return cb(null, name);
        console.log(
          `A file named "${name}" already exists (${++i}/${maxTries}).`
        );
  ...
  upload(req, res, async err => {
    ...
    req.files.forEach(async file => {
      ...
      let stream = fs.createReadStream(
        path.join(__dirname, "..", config.uploads.folder, file.filename)
      );
      ...
      stream.on("end", async () => {
        ...
        if (iteration === req.files.length) {
          return uploadsController.processFilesForDisplay(
            req,
            res,
            files,
            existingFiles,
...
uploadsController.processFilesForDisplay = async (
  req,
  res,
  files,
  existingFiles,
    ...
    return res.json({
      ...
      files: existingFiles.map(file => {
        return {
          ...
          deleteUrl: `${basedomain}/api/upload/delete/${token}/${file.id}`
        };
      })
    });
  }
  ...
  for (let file of files) {
    ...
    if (
      utils.imageExtensions.includes(ext) ||
      utils.videoExtensions.includes(ext)
    ) {
      file.thumb = `${basedomain}/thumbs/${file.name.slice(
        0,
        -ext.length
      )}.png`;
...
uploadsController.delete = async (req, res) => {
  if (req && (!req.headers || !req.headers.token) && req.params.token) {
    // get the token from URL params, required for ShareX deletion
...
uploadsController.deleteFile = file => {
  const ext = path.extname(file).toLowerCase();
  return new Promise((resolve, reject) => {
    fs.stat(
      path.join(__dirname, "..", config.uploads.folder, file),
      (err, stats) => {
        if (err) {
          return reject(err);
        }
